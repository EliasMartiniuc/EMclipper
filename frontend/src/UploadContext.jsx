...
export function UploadProvider({ children }) {
  ...
  const startProcessing = async () => {
    ...
    const canUpload = await checkSubscription();
    if (!canUpload && subscriptionStatus && !subscriptionStatus.can_upload) return;
  ...
  const startProcessingUrl = async (urlStr) => {
    if (!urlStr) {
      setError("Please enter a valid URL.");
      return;
    }
    if (!user) {
      setError("You must be logged in to process links.");
      return;
    }
    ...
    setHasClips(false);
    setProgress(100);
    setProgressText('Downloading video from URL...');
    setSpeedText('');
  ...
  const contextValue = useMemo(() => ({
    ...
    startProcessing, startProcessingUrl, stopProcessing, activeJobId, hasClips, subscriptionStatus, fetchSubscriptionStatus
